...
def User_info():
    ...
    try:
        with app:
            ...
            count = app.get_chat_photos_count(user_info)
            profile_photo = app.get_chat_photos(user_info)
            for item in profile_photo:
                app.download_media(item.file_id, file_name='{0}/profile_photos/{1}/'.format(os.getcwd(), user_info))
            if count == 0:
