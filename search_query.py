    ...
    try:
        ...
        with app:
            ...
            for message in app.search_messages(chat_id=chat_creds, query=search_query):
                messages = message.date.strftime('%Y-%m-%d %H:%M:%S') + ' from user ID: {}'.format(message.from_user.id) + '\n' + message.text
                message = '\033[1;94m' + message.date.strftime('%Y-%m-%d %H:%M:%S') + '\033[1;00m from user ID: \033[1;94m{}\033[1;00m'.format(message.from_user.id) + '\n' + message.text
                print(message)
