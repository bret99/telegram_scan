    ...
    try:
        ...
        try:
            with app:
                ...
                for message in app.search_messages(chat_id=chat_creds, filter=enums.MessagesFilter.ANIMATION):
                    print('\033[1;94m', message.date.strftime('%Y-%m-%d %H:%M:%S') + '\033[1;00m from user ID: \033[1;94m' + str(message.from_user.id) + '\033[00m')
                    from_users_list.append(message.date).strftime('%Y-%m-%d %H:%M:%S') + ' from user ID: ' + str(message.from_user.id)
